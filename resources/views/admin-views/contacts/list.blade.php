...
<div class="content container-fluid">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item" aria-current="page">{{('Customer Message')}}</li>
        </ol>
    </nav>
    <!-- Page Heading -->
    <div class="d-sm-flex align-items-center justify-content-between mb-2">
        <h1 class="h3 mb-0 text-black-50">{{('Customer')}} {{('Message')}} {{('List')}}</h1>
    </div>
                            ...
                            <form action="{{ url()->current() }}" method="GET">
                                <div class="input-group input-group-merge input-group-flush">
                                    ...
                                    <input id="datatableSearch_" type="search" name="search" class="form-control"
                                        placeholder="{{('Search_by_Name_or_Mobile_No_or_Email')}}" aria-label="Search orders" value="{{ $search }}" required>
                                    ...
                                    <td>
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                ...
                                                href="{{route('admin.contact.view',$contact->id)}}"> {{ ('View')}}</a>
                                                <a class="dropdown-item delete" style="cursor: pointer;"
                                                id="{{$contact['id']}}"> {{ ('Delete')}}</a>
                                            </div>
                                        </div>
                                    </td>
                                    <td>{{$contact['name']}}</td>
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        data: {id: id},
                        success: function () {
                            toastr.success('{{('Contact_deleted_successfully')}}');
                            location.reload();
                        }
                    });
                }
