...
class ProductController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        $p->specification = $request->specification[array_search('en', $request->lang)];
        ...
        $p->thumbnail_back = ImageManager::upload('product/thumbnail/', 'png', $request->back_thumbnail);
        // return 'works point 3';
        //combinations end
    ...
    public function colorImage($id)
    {
        ...
        return view('admin-views.product.color-images',compact('product'));
    }

    public function colorImageAdd(Request $request)
    {
        $product = Product::find($request->product_id);
        $color_images = json_decode($product->color_images, true) ?? [];
        if ($request->has('color') && $request->hasFile('images')) {
            $color = $request->color;
            $new_images = [];
            // Upload the new images
            foreach ($request->file('images') as $img) {
                $new_images[] = ImageManager::upload('product/', 'png', $img);
            }
            // Check if the color already exists in the array
            $color_exists = false;
            foreach ($color_images as &$color_image) {
                if ($color_image['color'] == $color) {
                    // If color exists, add the new images to the existing ones
                    $color_image['images'] = array_merge($color_image['images'], $new_images);
                    $color_exists = true;
                    break;
                }
            }
            // If color doesn't exist, add a new entry
            if (!$color_exists) {
                $color_images[] = [
                    'color' => $color,
                    'images' => $new_images
            ...
            // Save the updated color_images to the product
            $product->color_images = json_encode($color_images);
            $product->save();
            Toastr::success('Product Color Images Added Successfully.');
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        $product->specification = $request->specification[array_search('en', $request->lang)];
        ...
        if ($request->file('back_thumbnail')) {
            $product->thumbnail_back = ImageManager::update('product/thumbnail/', $product->back_thumbnail, 'png', $request->file('back_thumbnail'));
        }
        //combinations end
    ...
    public function thumbnailt()

    {

        return view('admin-views.product.thumbnailt');

    }

    

    public function insert_thumbnailt(Request $request)

    {
      ...
      foreach ($request->file('image') as $key => $img) {
            ...
            $imageName = $time . '-' .time().$key.'.'.$img->extension();

            $img->move(base_path() . '/storage/product/thumbnail/', $imageName);

            DB::insert('insert into thumbnailt(add_by,image_url) values(?,?)',['0',$imageName]);

        }

        Toastr::success('Image Updload successfully!');
...

    {
        ...
        return view('admin-views.product.product_image');
...

    {
      ...
      foreach ($request->file('image') as $img) {
                ...
                DB::insert('insert into product_image(add_by,image_url) values(?,?)',['0',$product_images]);
    ...
    function product_image_list(Request $request)

    {
...

        {
            ...
            $br = product_image::where(function ($q) use ($key) {
        ...
        return view('admin-views.product.product_image_list', compact('br','search'));

    }

    

function thumbnailt_list(Request $request)

    {
...

        {
            ...
            $br = thumbnailt::where(function ($q) use ($key) {
        ...
        return view('admin-views.product.thumbnailt_image', compact('br','search'));
    ...
    public function delete_thumbnailt(Request $request)

    {

        $brand = thumbnailt::find($request->id);
...

    {
        ...
        $brand = product_image::find($request->id);
...

    {
        ...
        DB::table('product_discount')->where('id','1')->update(['range_1'=>$request->range_1,'range_2'=>$request->range_2,'range_3'=>$request->range_3,'range_amount_1'=>$request->range_amount_1,'range_amount_2'=>$request->range_amount_2,'range_amount_3'=>$request->range_amount_3]);
    ...
    public function update_charge1(Request $request)

    {

        DB::table('product_discount')->where('id','2')->update(['range_1'=>$request->range_4,'range_2'=>$request->range_5,'range_3'=>$request->range_6,'range_amount_1'=>$request->range_amount_4,'range_amount_2'=>$request->range_amount_5,'range_amount_3'=>$request->range_amount_6]);
