...

{
    ...
    public static function get_cart_web($group_id = null)
    ...
    public static function get_cart_mobile($group_id = null)
    ...
    public static function get_cart_group_ids123($user_id)

    {
        ...
        // } else {
            ...
            $cart_ids = Cart::where(['customer_id' => $user_id])->groupBy('cart_group_id')->pluck('cart_group_id')->toArray();
...

    {
        ...
        if(session()->has('address_id'))
        {
            ...
            {
                $cost=$delivery_charge->delivery_charge_2;
            }
        }
        else
        {
    ...
    public static function cart_grand_total1($cart_group_id = null)
    ...
    public static function coupon_grand_total($cart_group_id = null)
    ...
    public static function web_cart_grand_total($cart_group_id = null)
    ...
    public static function cart_clean_mobile($group_id)

    {

        // $cart_ids = CartManager::get_cart_group_ids($request);

        CartShipping::where('cart_group_id',$group_id)->delete();

        Cart::where('cart_group_id', $group_id)->delete();
...

    {
        ...
        if ($request->has('color')) {
            ...
            $str2 = Color::where('code', $request['color'])->first()->name;

            $variations['color'] = $str2;
        ...
        foreach (json_decode($product->choice_options) as $key => $choice) {
            ...
            $choices[$choice->title] = $request->variant;

            $variations[$choice->title] = $request->variant;

            if ($str != null) {

                $str .= '-' . str_replace(' ', '', $request->variant);
    ...
    public static function add_to_cart_web($request, $from_api = false)
    {
        $str = '';
        $variations = [];
        $price = 0;
        ...
        if ($str != null) {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if ($str) {
                    if ($str2) {
                        if (json_decode($product->variation)[$i]->type == $MainStr) {
                            $price = json_decode($product->variation)[$i]->price;
                            // dd($price);
                                if (json_decode($product->variation)[$i]->qty < $request['quantity']) {
                                    return [
        ...
        $tax = Helpers::tax_calculation($withDiscount, $product['tax'], 'percent');
        // $tax = $product->tax;
        //generate group id
        if ($user == 'offline') {
            $check = session('offline_cart');
