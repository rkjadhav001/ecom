...
class Helpers
{
        ...
        if (isset($coupon)) {
            ...
            if($coupon['seller_id']=='0' || $coupon['seller_id']==0)
            {
                ...
                foreach (CartManager::get_cart(CartManager::get_cart_group_ids($request)) as $cart) {
                    ...
                    if($cart['seller_id']==$coupon['seller_id'])
                    {
    ...
    public static function product_data_formatting($data, $multi_data = false)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                $item['colors'] = Color::whereIn('code', json_decode($item['colors']))->orderBy('name')->get(['name', 'code']);
                $attributes = [];
            ...
            $data['colors'] = Color::whereIn('code', json_decode($data['colors']))->orderBy('name')->get(['name', 'code']);
            $attributes = [];
    ...
    public static function send_push_notif_to_coupon($data)
    {
        $key = BusinessSetting::where(['type' => 'push_notification_key'])->first()->value;
    ...
    {
        $mpdf = new \Mpdf\Mpdf(['default_font' => 'FreeSerif', 'mode' => 'utf-8', 'format' => [400, 236]]);
